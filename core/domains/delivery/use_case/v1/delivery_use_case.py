...
class TrackDeliveryUseCase:
    @inject.autoparams()
    def __init__(self, delivery_repo: DeliveryRepository, update_use_case: UpdateDeliveryUseCase):
        self._delivery_repo = delivery_repo
        self._update_use_case = update_use_case

    def execute(self, dto: TrackDeliveryDto) -> UseCaseSuccessOutput | UseCaseFailureOutput:
        delivery = self._delivery_repo.find_by_parcel_company_id_and_number(parcel_company_id=dto.carrier_id,
                                                                            parcel_num=dto.tracking_number)
        ...
        if token_response.raise_for_status():
            return UseCaseFailureOutput(type_="0100", message="Failed get token", code=HTTPStatus.NOT_FOUND)
        ...
        if track_response.raise_for_status():
            return UseCaseFailureOutput(type_="0100", message="Failed tracking", code=HTTPStatus.NOT_FOUND)

        status = track_response.json()["data"]["track"]["lastEvent"]["status"].get("code")

        update_dto = UpdateDeliveryDto(
            delivery_id=delivery.id,
            type=None,
            status=DeliveryStatus.DELIVERY_DONE.value[0] if status == "DELIVERED" else DeliveryStatus.DELIVERING.value[
                0],
